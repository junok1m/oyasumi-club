...
export async function generateMetadata({
  ...
  const description =
    post.excerpt?.trim() || plainBody.slice(0, 120) || "シドニーで働く女の子向けのQ&Aです。";
  const canonicalUrl = `${SITE_URL}/qna/${getCanonicalSlug(post)}`;
  return {
    title: `${post.title} | Q&A | おやすみクラブ`,
    description,
  ...
  return (
    <main className="-mb-24 min-h-dvh bg-[#fff3f8] pb-32 text-[#5f4d5c]">
      <div className="mx-auto w-[92%] max-w-5xl py-6 md:w-[86%] md:py-10">
        <div className="mb-8">
          <Link href="/qna" className="block w-fit text-[12px] text-pink-500">
            ← Q&A一覧へ戻る
          </Link>
        </div>

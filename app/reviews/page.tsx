    ...
    <div className="mb-8">
      <div className="flex gap-2 overflow-x-auto pb-1 pt-1 whitespace-nowrap">
        {industries.map((item) => {
          ...
          return (
            <Link
              key={item.value}
              href={buildReviewsUrl({ industry: nextIndustry, sort, q, location })}
      ...
      <div className="mt-4 flex justify-end text-[12px]">
        <div className="flex border border-pink-200 bg-pink-50">
          <Link
            href={buildReviewsUrl({ industry, sort: "latest", q, location })}
            className={
              sort === "latest"
          ...
          <Link
            href={buildReviewsUrl({ industry, sort: "views", q, location })}
            className={
              sort === "views"
                ? "bg-pink-200 px-3 py-1.5 text-[#4f3a4f]"
                : "px-3 py-1.5 text-[#b28aa8]"
...
async function getPosts(
  ...
  let query = supabase
    ...
    .select(
      `
      id, title, category, industry, location,
...
export async function generateMetadata({
  ...
  return {
    ...
    description,
    alternates: { canonical: "https://www.oyasumi-club.com/reviews" },
    openGraph: {
      title,
      description,
      url: "https://www.oyasumi-club.com/reviews",
      type: "website",
    },
  };
}
      ...
      <div className="mx-auto w-[92%] max-w-5xl py-8">
        ...
        <div className="space-y-6">
          ...
          <LocationPills
            ...
            queryMode
          />
          <ReviewsFilter industry={industry} sort={sort} q={q} location={location} />
        </div>
              ...
              <div key={post.id} className="py-4">
                ...
                <div className="ml-8 mt-1.5 flex flex-wrap items-center gap-x-2 gap-y-1 text-[12px] text-[#9b7892]">
                  {industryLabel(post.industry) && (
                    <Link
                      href={buildReviewsUrl({ industry: post.industry || "all", sort, q, location })}
                  ...
                  {post.location && (
                    <Link
                      href={buildReviewsUrl({ industry, sort, q, location: post.location })}
        ...
        {data.total_pages > 1 && (
          <div className="mt-10 flex items-center justify-center gap-2 text-sm">
            {data.page > 1 && (
              <Link
                href={buildReviewsUrl({ industry, sort, page: data.page - 1, q, location })}

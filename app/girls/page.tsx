...
<div className="my-6">
  ...
  />
                    ...
                    <div className="mt-8 flex flex-wrap justify-center gap-3">
                        ...
                        <Link
                            href="/board/write?category=qa&audience=women"
            ...
            <section className="mx-auto grid w-[92%] max-w-5xl grid-cols-4 gap-3">
                {[
                    ["47", "口コミ", "/girls/reviews", "働きやすさ"],
                    ["88", "Q&A", "/girls/qna", "相談・質問"],
                    ["25", "求人", "/girls/jobs", "募集中"],
                    ["12", "記事", "/girls/blog", "ノウハウ"],
  ...
  <div className="divide-y divide-pink-100">
    {(qnaPosts ?? []).map((post) => {
      ...
      return (
        <Link
          key={post.id}
          href={`/girls/qna/${prettySlug(post)}#comments`}
...
<section className="mx-auto mt-14 w-[92%] max-w-5xl">
  ...
  <div className="divide-y divide-pink-100">
    {(jobs ?? []).map((job) => (
      <Link
        key={job.id}
        href={`/girls/jobs/${prettySlug(job)}`}
        className="group block py-4 transition hover:bg-pink-50/60"
      >
        <div className="min-w-0">
          <div className="flex items-center gap-3 text-xs">
  ...
  <div className="divide-y divide-pink-100">
    {(reviews ?? []).map((review) => (
      <Link
        key={review.id}
        href={`/girls/reviews/${prettySlug(review)}`}
        className="group block py-4 transition hover:bg-pink-50/60"
      >
        <div className="min-w-0">
          <div className="flex items-center gap-3 text-xs">
        ...
        まだ口コミがありません。
            ...
            <section className="mx-auto mt-14 w-[92%] max-w-5xl">
                <div className="mb-5 flex items-end justify-between">
                    ...
                    <Link href="/girls/blog" className="text-sm font-bold text-pink-500">
                        もっと見る →
                    </Link>
                </div>
                ...
                <div className="-mx-4 overflow-x-auto px-4 pb-2">
                    <div className="flex gap-4">
                        {(tipsPosts ?? []).map((post) => (
                            <Link
                                key={post.id}
                                href={`/girls/blog/${prettySlug(post)}`}

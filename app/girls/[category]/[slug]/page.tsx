...
import QaComments from "@/app/board/[slug]/QaComments";
...
export async function generateMetadata({
  ...
  const post = await getPost({ category, prettySlug: slug });
  ...
  const description =
    ...
    `シドニーで働く女の子向けの${config.label}です。`;

  const canonicalUrl = `${SITE_URL}/girls/${category}/${getCanonicalSlug(post)}`;

  return {
    title: `${post.title} | 女の子向け${config.label} | おやすみクラブ`,
    description,
    alternates: {
...
export default async function GirlsPostDetailPage({
  params,
}: {
  params: Promise<{ category: string; slug: string }>;
}) {
    ...
    <main className="-mb-24 min-h-dvh bg-[#fff3f8] pb-32 text-[#5f4d5c]">
      <div className="mx-auto w-[92%] max-w-5xl py-6 md:w-[86%] md:py-10">
        <div className="mb-8">
          <Link
            href={`/girls/${category}`}
            className="block w-fit text-[12px] text-pink-500"
          >
            ← {config.backLabel}
          </Link>
        </div>
        ...
        <article>
          ...
          <div
            className="
              ...
              [&_li]:my-1
              [&_a]:text-[#a8739b] [&_a]:underline [&_a]:underline-offset-2 [&_a]:cursor-pointer
          ...
          {post.category === "qa" && <QaComments postId={post.id} />}
